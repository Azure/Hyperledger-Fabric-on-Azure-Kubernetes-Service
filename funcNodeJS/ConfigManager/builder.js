module.exports = {
  addNodesNameToOrganizationsGateway: async function(gatewayObject, nodesNameArray, orgMSPID, nodesType) {
    try {
  ...
  addNodesObjectsToGateway: async function(gatewayObject, nodesType, nodesNameArray, nodesDnsArray, hlfTlsCaCert) {
    try {
      for (i = 0; i <= (nodesNameArray.length - 1); i++) {
        // Get new object without "links" to parent
        let nodeObject = JSON.parse(JSON.stringify(gatewayObject[nodesType]['{nodeName0}']))

        // hostnameOverride/ssl-target-name-override should contain only full qualified domain name of the node
        const pureNodeDNS = nodesDnsArray[i].replace("grpcs://", "").replace(":443", "")
  ...
  addNodesObjectsToNodes: async function(nodeObject, nodesMspId, nodesNameArray, nodesBase64certArray, nodesDnsArray, hlfMemberType) {
    try {
      const nodesArray = []
      for (i = 0; i <= (nodesNameArray.length - 1); i++) {
        // Get new object without "links" to parent
        ...
        newNodeObject.pem = nodesBase64certArray[i]
  ...
  getDnsRecords: async function(client) {
    try {
      let nodesResult = await client.apis.extensions.v1beta1.namespaces(namespaceNames.NODES).ingresses().get({})
      ...
      let caResult = await client.apis.extensions.v1beta1.namespaces(namespaceNames.CA).ingresses().get({})
  ...
  getHlfTlsCaCert: async function(client) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.CA).secrets('hlf-tlsca-idcert').get({})
  ...
  getNodesName: async function(client, hlfMemberType, hlfBackendDbType, hlfOrgName) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.NODES).pods.get({})

      let pods = result.body.items.filter(pod => pod.metadata.name.startsWith(hlfMemberType))
  ...
  getNodesPemCert: async function(client, hlfMemberType) {
    try {
      const ordererRegExp = /hlforderer\d-idcert/
      const peerRegExp = /hlfpeer\d-idcert/
      secretMask = hlfMemberType == 'orderer' ? ordererRegExp : peerRegExp

      let result = await client.api.v1.namespaces('hlf').secrets.get({})

      result = result.body.items.filter(secret => {
        return (secret.metadata.name.search(secretMask) !== -1)
      })

      let nodesBase64certArray = []
      result.forEach(cert => {
        nodesBase64certArray.push(cert.data['cert.pem'])
      })
      return nodesBase64certArray
    } catch (err) {
      return err
    }
  },
  getAdminPemKey: async function(client) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.ADMIN).secret('hlf-admin-idkey').get({})
  ...
  getAdminPemCert: async function(client) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.ADMIN).secret('hlf-admin-idcert').get({})
  ...
  getAdminPemTlsKey: async function(client) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.ADMIN).secret('hlf-admin-tls-idkey').get({})
  ...
  getAdminPemTlsCert: async function(client) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.ADMIN).secret('hlf-admin-tls-idcert').get({})
  ...
  getCaPemCert: async function(client) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.CA).secret('hlf-ca-idcert').get({})
  ...
  getNodesMspId: async function(client, hlfMemberType, hlfBackendDbType) {
    try {
      let result = await client.api.v1.namespaces(namespaceNames.NODES).pods.get({})
      result = result.body.items.filter(pod => pod.metadata.name.startsWith(hlfMemberType))
      ...
      const envName = hlfMemberType == 'orderer' ? 'ORDERER_GENERAL_LOCALMSPID' : 'CORE_PEER_LOCALMSPID'

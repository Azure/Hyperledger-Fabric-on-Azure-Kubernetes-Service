module.exports = async function(context, req) {
  ...
  const nodesNameArray = await builder.getNodesName(client, hlfMemberType, hlfBackendDbType, hlfOrgName)
  ...
  async function getGateWay() {
    let gatewayString = (fs.readFileSync(`${__dirname}/models/gateway.json`)).toString('utf8')
    gatewayString = gatewayString.replace(/{orgCA}/g, `${hlfOrgName}CA`)
    gatewayString = gatewayString.replace(/{ca}/g, `${serverCaName}.${hlfOrgName}`)
    ...
    gatewayObject.certificateAuthorities[`${hlfOrgName}CA`].tlsCACerts.pem = hlfTlsCaCert
    gatewayObject = await builder.addNodesObjectsToGateway(gatewayObject, nodesType, nodesNameArray, nodesDnsArray, hlfTlsCaCert)
    gatewayObject = await builder.addNodesNameToOrganizationsGateway(gatewayObject, nodesNameArray, hlfOrgName, nodesType)
    const gateway = JSON.stringify(gatewayObject, null, 2)
    return gateway
  }

  async function getNode(nodeNumber) {
    let nodeString = (fs.readFileSync(`${__dirname}/models/nodes.json`)).toString('utf8')
    let nodeObject = JSON.parse(nodeString)
    const nodesBase64certArray = await builder.getNodesPemCert(client, hlfMemberType)
    const nodesMspId = await builder.getNodesMspId(client, hlfMemberType, hlfBackendDbType)
    const nodesArray = await builder.addNodesObjectsToNodes(nodeObject, nodesMspId, nodesNameArray, nodesBase64certArray, nodesDnsArray, hlfMemberType)
  ...
  async function getAdmin() {
    let adminString = (fs.readFileSync(`${__dirname}/models/admin.json`)).toString('utf8')
  ...
  async function getCA() {
    let caString = (fs.readFileSync(`${__dirname}/models/CA.json`)).toString('utf8')
